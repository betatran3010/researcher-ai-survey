...
test('strict exposure threshold: 5000 does not count and 5001 does',()=>{withClock(c=>{const m=loadViewportModule();c.set(0);setGeometry(m,'font',{pages:PAGES,viewportTop:0,viewportHeight:100});m.startViewportTracking('font');m.markPdfContentReady('font');c.advance(5000);m.tickSegment('font');m.stopViewportTracking('font');assert.equal(m.DATA.timing.font.region_exposed_5s_count,0);});withClock(c=>{const m=loadViewportModule();c.set(0);setGeometry(m,'food',{pages:PAGES,viewportTop:0,viewportHeight:100});m.startViewportTracking('food');m.markPdfContentReady('food');c.advance(5001);m.tickSegment('food');m.stopViewportTracking('food');assert.equal(m.DATA.timing.food.region_exposed_5s_count,1);});});
test('viewport spanning adjacent halves credits both regions proportionally',()=>withClock(c=>{const m=loadViewportModule();c.set(0);setGeometry(m,'font',{pages:PAGES,viewportTop:50,viewportHeight:100});m.startViewportTracking('font');m.markPdfContentReady('font');c.advance(10002);m.tickSegment('font');m.stopViewportTracking('font');assert.equal(m.DATA.timing.font.region_exposed_5s_count,2);assert.ok(m.DATA.timing.font.pdf_exposure_proportion_5s>0); }));
test('hidden and unfocused time are excluded',()=>withClock(c=>{const m=loadViewportModule();c.set(0);setGeometry(m,'font',{pages:PAGES,viewportTop:0,viewportHeight:100});m.startViewportTracking('font');m.markPdfContentReady('font');c.advance(3000);m.tickSegment('font');setGeometry(m,'font',{pages:PAGES,viewportTop:0,viewportHeight:100,visible:false});m.updateViewportSegment('font');c.advance(5000);m.tickSegment('font');m.stopViewportTracking('font');assert.equal(m.DATA.timing.font.region_exposed_5s_count,0);}));
