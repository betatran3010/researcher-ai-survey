...
## Current development Google Cloud resources
...
These identifiers refer to the current development project. If the lab deploys the survey under a different Google Cloud project, replace the project ID, bucket names, region, service-account email, and service URL throughout.
...
curl -X POST http://localhost:3000/api/assign-condition \
  ...
  -d '{"stable_participant_id":"test-participant-1","research_role":"First-year PhD student"}'
```
...
`--source .` builds the container for you (Cloud Build, via the Node buildpack). The repository includes a Dockerfile. When deploying from source, Cloud Build will build the application using that Dockerfile. Keep the Dockerfile and .dockerignore in the repository. `--allow-unauthenticated` is required so participants (who aren't Google-authenticated) can load the survey. The deploy prints a service URL like `https://researcher-ai-survey-xxxxx-uw.a.run.app`.
