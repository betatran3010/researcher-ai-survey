...
(one set of the first eight per paper in the fixed 3-paper pool, plus four
survey-wide totals):
...
Three fields are dropped entirely, including from the JSON export — not
excluded-but-recoverable like the above, but actually removed by
`cleanRecord()` before the CSV is built (the JSON export route does not call
`cleanRecord()` at all, so even these three remain in the JSON export) —
because they were write-once placeholders at session start that nothing in
the frontend ever read or wrote again; the real data they might have
suggested already lives elsewhere:
...
- `responses_json` — the only place every raw response key (including ones
  ...
  the CSV.
- `copy_events_json` / `paste_events_json` — each event's exact source text
