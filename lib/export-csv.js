...
for (const paperId of PAPER_IDS) {
  PER_PAPER_COLUMNS.push(
    ...
    `${paperId}_pdf_exposure_proportion_5s`,
    `${paperId}_navigation_sequence`,
    `${paperId}_navigation_transition_count`,
...
row.ct_repeat_exact_match =
  ...
  for (const paperId of PAPER_IDS) {
    ...
    row[`${paperId}_pdf_exposure_proportion_5s`] =
      t.pdf_exposure_proportion_5s ?? '';

    row[`${paperId}_navigation_sequence`] =
      t.navigation_sequence ?? '';

    row[`${paperId}_navigation_transition_count`] =
      t.navigation_transition_count ?? '';
